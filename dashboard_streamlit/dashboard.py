...
def layout():
    ...
    st.write(
        "This dashboard shows Housing prices comparison from SCB API. "
    )

    st.markdown("## housing_prices")
    cols = st.columns(3)

    with cols[0]:
        st.metric(label="Total", value=df["housing_prices"])  ############

    with cols[1]:
        st.metric(
            label="In ",
            value=df.query("WORKPLACE_CITY == 'Riket'")["VACANCIES"],
        )
